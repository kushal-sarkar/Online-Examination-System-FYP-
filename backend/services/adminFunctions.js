...
let trainerRegister = (req,res,next)=>{
    ...
    if(req.user.type==='ADMIN'){
        ...
        req.check('contact','Invalid contact number').isLength({min : 14,max :14}).isNumeric({no_symbols: false});
        ...
        if(errors){
            ...
            if(_id!=null){
                ...
                { 
                    ...
                    res.status(500).json({
                        success : false,
                        message : "Unable to update Teacher's Profile"
                    })
                })
            }
            else{
                    ...
                    if(!user){
                        tool.hashPassword(password).then((hash)=>{
                            ...
                            tempdata.save().then(()=>{
                                res.json({
                                    success : true,
                                    message : `Teacher's Profile created successfully!`
                                })
                            }).catch((err)=>{
                                console.log(err);
                                res.status(500).json({
                                    success : false,
                                    message : "Unable to create Teacher's Profile"
                                })
                            })
                        }).catch((err)=>{
                            console.log(err);
